...
## 0️⃣ 先修导读：API 网关到底是什么？

> 本阶段解决一个核心问题：**客户端（如小程序/App）要访问多个微服务，怎么统一管理？**
...
**问题**：
1. **端口暴露**：每个服务的端口都暴露给客户端，安全风险大
2. **重复鉴权**：每个服务都要独立做登录验证
3. **跨域问题**：小程序调用多个域名，存在跨域
4. **协议不统一**：有的服务用 REST，有的用 gRPC

#### 🏭 有网关之后：所有请求先到网关

```
[客户端] --> [API网关:443] --> [用户服务A:8001]
                         --> [商品服务B:8002]
                         --> [订单服务C:8003]
```

**好处**：
1. 客户端只暴露一个端口（443 HTTPS）
2. 网关统一鉴权，后端服务不用管
3. 网关处理跨域
4. 网关做流量控制、安全防护
...
**好处**：
- Web BFF 返回完整数据（网页展示需要）
- 小程序 BFF 只返回必要字段（节省流量，加快渲染）
- 前端不再关心后端服务拆分细节
...
用户点击"下单"，网络超时 → 前端重试 → **同一订单下了两次！**

**幂等性 = 同一个请求执行一次和执行多次，结果一样。**

| 操作 | 幂等？| 原因 |
|------|------|------|
| GET 查询 | ✅ 幂等 | 只是读取，不会改变数据 |
| PUT 更新 | ✅ 幂等 | 同样的数据更新多少次都一样 |
| DELETE 删除 | ✅ 幂等 | 删一次和删多次结果一样 |
| POST 下单 | ❌ 不幂等 | 每次下单都创建新订单 |
| POST 扣款 | ❌ 不幂等 | 每次都扣钱 |

**怎么保证幂等？**
- 下单接口：前端生成**唯一订单号**，后端用订单号做去重
- 支付回调：用**支付流水号**做幂等
