...
## 0️⃣ 核心概念先修：IoC + 依赖注入 + AOP 入门

> **必读！** 在学习 NestJS 之前，你需要先理解三个最核心的概念。它们贯穿整个 NestJS 框架，理解了它们，你就理解了 NestJS 一半的设计思想。
...
想象你要做一套管理系统，你本来需要：
- 自己写代码（需要懂 Java、懂架构、懂数据库）
- 自己买服务器
- 自己维护系统

**太累了**。于是你找来一个**外包公司**，说"帮我做个管理系统"，外包公司说"没问题！"然后他们内部协调工程师、设计师、运维，你只需要等结果。

在这个例子里：**控制权发生了反转**。
- **传统方式**：你控制一切（代码怎么写、用什么技术栈）
- **IoC 方式**：你把"做什么"告诉框架，框架内部决定"怎么做"

#### 📖 NestJS 中的 IoC

NestJS 有一个 **IoC 容器（控制反转容器）**，它的工作是：
1. 记住"有哪些服务可以被创建"
2. 记住"谁需要依赖谁"
3. 当需要一个服务时，自动创建它（如果还没创建）并注入

**好处**：你不需要手动 `new UserService()`，NestJS 帮你搞定，而且确保每个需要的地方拿到的是正确的实例。

---

### 🔍 2. 依赖注入（DI）：NestJS 怎么知道该创建什么？
...
这里 `UserController` **依赖** `UserService`。没有 `UserService`，`UserController` 就没法工作。

**依赖注入 = 由外部（NestJS）把 `UserService` 的实例"塞给" `UserController`**。
...
问题：
- 如果 `UserService` 依赖 `PrismaService`（数据库服务），你还得手动创建它
- 如果 `PrismaService` 又依赖 `ConfigService`，你得一直往下找
- 如果测试时想换成 `MockUserService`，要改很多地方

```typescript
// ✅ 依赖注入方式（NestJS 推荐）
@Controller('users')
export class UserController {
  constructor(private userService: UserService) {} // NestJS 自动注入
}
```

**你只需要声明"我需要什么"，NestJS 自动找到正确的实例给你。**

---

### 🔍 3. AOP 面向切面编程：拦截器/守卫/管道的本质
...
```
请求 → [记录日志] → [鉴权] → [参数验证] → [业务逻辑] → [异常处理] → [格式化响应] → 响应
```

在这个流水线上，**日志记录、鉴权、参数验证、异常处理** 都是"横切"关注点——它们不是业务核心，但每个请求都需要经过。

**AOP = 把这些横切关注点"抽出来"，不让它们污染业务逻辑代码。**

NestJS 的 AOP 实现：
| 组件 | 职责 | 介入时机 |
|------|------|---------|
| Middleware（中间件）| 请求预处理 | 路由匹配之前 |
| Guard（守卫）| 权限检查 | 路由匹配之后、处理器之前 |
| Pipe（管道）| 参数转换/验证 | 参数进入处理器之前 |
| Interceptor（拦截器）| 前后置逻辑 | 处理器前/后 |
| Exception Filter | 异常统一处理 | 发生异常时 |

#### 🍜 一句话总结

- **IoC 容器**："我需要这个服务" → 框架帮我创建
- **依赖注入**：把服务实例"注入"到需要它的地方
- **AOP**：把横切关注点（日志、鉴权、验证）从业务逻辑中分离出来
...
**Q: IoC 和 DI 是什么关系？**
A: IoC 是一种**设计思想**（控制权反转），DI 是这种思想的一种**实现方式**（把依赖从外部注入进来）。NestJS 用 DI 实现了 IoC。

**Q: 一定要用 NestJS 的 IoC 吗？我自己 new 不行吗？**
A: 对于小项目，自己 new 完全没问题。但 NestJS 的 IoC 让你不用手动管理复杂依赖关系，而且支持单例（省内存）、生命周期管理（请求结束自动清理）、Mock 测试等高级功能。

**Q: AOP 听起来很复杂，我真的需要吗？**
A: 你其实每天都在用 AOP！路由守卫检查登录状态 = AOP；请求参数自动验证 = AOP；统一错误处理 = AOP。NestJS 只是把这些模式化了，让你可以复用别人的实现（如 `@UseGuards()`）。
